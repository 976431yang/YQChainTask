...
public class YQChainTask {
    ...
    private var nextTask: DDChainTask?
    private var _action: ((DDChainTask) -> Void)?
    
    //如果只初始化一个任务，可以忽略task
    public init(action:@escaping ((DDChainTask) -> Void)) {
        self._action = action
        self.nextTask = nil
    }
    ...
    @discardableResult
    public func next(_ nextAction: @escaping (DDChainTask) -> Void ) -> DDChainTask {
        let newTask = DDChainTask(action: nextAction)
